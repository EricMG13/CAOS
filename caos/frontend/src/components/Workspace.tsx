...
type RunControls = { caseId: string; run: RunRecord | null; startRun: (event: FormEvent<HTMLFormElement>) => void; acceptRun: () => void };
...
export default function Workspace({ destination }: { destination: string }) {
  ...
  const createCase = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setCases((previous) => [created, ...previous]); setCaseId(created.id); element.reset();
  ...
  const upload = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const element = event.currentTarget;
    const form = new FormData(element);
    try { await request(`/api/cases/${caseId}/sources`, { method: "POST", body: form }); await refreshCase(); element.reset(); }
  ...
  const renderDestination = () => {
    ...
    switch (active) {
      case "Cases": return <CasesView cases={cases} selectedCase={selectedCase} caseId={caseId} setCaseId={setCaseId} createCase={createCase} upload={upload} run={run} startRun={startRun} acceptRun={acceptRun} />;
      ...
      case "Deep-Dive": return <DeepDive selectedCase={selectedCase} caseId={caseId} run={run} startRun={startRun} acceptRun={acceptRun} />;
...
function RunForm({ caseId, startRun }: { caseId: string; startRun: (event: FormEvent<HTMLFormElement>) => void }) {
  const id = useId();
  return <form onSubmit={startRun}><div className="field"><label htmlFor={`${id}-pathway`}>Purpose</label><select id={`${id}-pathway`} name="pathway" defaultValue="EARNINGS_UPDATE">{pathways.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></div><div className="field"><label htmlFor={`${id}-depth`}>Depth</label><select id={`${id}-depth`} name="depth" defaultValue="screen"><option value="screen">Screen</option><option value="full">Full</option></select></div><button className="button primary" type="submit" disabled={!caseId}>Compile and run</button></form>;
}

function RunStatusBadge({ run }: { run: RunRecord | null }) {
  if (!run) return null;
  return <span className={`status ${run.status === "succeeded" ? "success" : run.status === "failed" ? "critical" : "warning"}`}>{run.status}</span>;
}

// fallow-ignore-next-line complexity -- empty/dag/error/accept/paused are sibling render branches that can appear together; splitting them adds indirection without removing a decision
function RunStatus({ run, acceptRun }: { run: RunRecord | null; acceptRun: () => void }) {
  if (!run) return <div className="empty">No current execution. Select a purpose and depth to create an immutable plan.</div>;
  return <><div className="dag">{run.nodes.map((node) => <div className={`dag-node ${node.status}`} key={node.id}><strong>{node.module_id}</strong><div className="muted">{node.status}</div></div>)}</div>{run.error && <p className="error">{run.error.message || "Run exception"}</p>}{run.status === "succeeded" && <button className="button primary" style={{ marginTop: 15 }} onClick={acceptRun}>Accept analytical snapshot</button>}{run.status === "paused" && <div className="callout warning" style={{ marginTop: 15 }}>Material exception: upload governed source material before execution.</div>}</>;
}

// Inline execution: compile and watch a route without leaving the analytical surface.
function InlineRun({ caseId, run, startRun, acceptRun }: RunControls) {
  return <section className="panel span-12"><div className="panel-header"><h2>Inline execution</h2><div className="top-actions"><RunStatusBadge run={run} /><span className="eyebrow">LIVE</span></div></div><div className="panel-body" style={{ display: "flex", gap: 18, alignItems: "flex-start" }}><div style={{ minWidth: 220 }}><RunForm caseId={caseId} startRun={startRun} /></div><div style={{ flex: 1, minWidth: 0 }}><RunStatus run={run} acceptRun={acceptRun} /></div></div></section>;
}

function RunConsole({ caseId, run, startRun, acceptRun }: RunControls) {
  return <div className="grid"><section className="panel span-4"><div className="panel-header"><h2>Compile route</h2><span className="eyebrow">IMMUTABLE PLAN</span></div><div className="panel-body"><RunForm caseId={caseId} startRun={startRun} /><div className="callout" style={{ marginTop: 14 }}>CP-PARSE is prepended to every route and CP-0 consumes its digest.</div></div></section><section className="panel span-8"><div className="panel-header"><h2>Persisted DAG</h2><RunStatusBadge run={run} /></div><div className="panel-body"><RunStatus run={run} acceptRun={acceptRun} /></div></section></div>;
}

function DeepDive({ selectedCase, caseId, run, startRun, acceptRun }: { selectedCase: CaseRecord | null } & RunControls) {
