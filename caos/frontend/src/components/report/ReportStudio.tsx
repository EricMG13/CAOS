...
import { api as request, type CaseRecord, type SourceRecord } from "../../lib/api";
...
type RegistryResponse = { version: string; digest: string; definitions: { assumption_id: string; label?: string; periods?: string[] }[] };
...
export default function ReportStudio({ caseId, role, selectedCase, onDraftStateChange }: { caseId: string; role: string; selectedCase: CaseRecord | null; onDraftStateChange: (dirty: boolean) => void }) {
  ...
  const [scenarioForm, setScenarioForm] = useState({ assumptionId: "", case: "BASE", periodId: "FY2025", value: "" });
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      setWorkspace(next); setBlocks(nextBlocks); setModelSelection(next.current?.content.model_selection || next.model_eligibility.default_model_selection); setSelectedBlockId(nextBlocks[0]?.block_id || ""); setSources(sourceResult.value); setEvidenceError(sourceResult.error); setSaveState(next.current ? { kind: "SAVED", version: next.current.version } : { kind: "IDLE" });
      const buildId = next.model_eligibility.active_revision?.build_id || next.model_eligibility.application_build?.build_id;
      if (buildId) {
        void request<RegistryResponse>(`/api/cases/${caseId}/models/assumption-registry`, {}, signal).then((value) => {
          if (generation !== loadGeneration.current || currentScope.current !== scope) return;
          setRegistry(value); setScenarioForm((current) => ({ ...current, assumptionId: current.assumptionId || value.definitions[0]?.assumption_id || "", periodId: current.periodId || value.definitions[0]?.periods?.[0] || "FY2025" }));
        }).catch(() => { if (generation === loadGeneration.current) setRegistry(null); });
    ...
    <section className="panel report-compose" aria-labelledby="report-compose-title">
      ...
      <div className="panel-body report-editor-scroll">
        {selectedFrozen ? <div className="flow"><p className="status warning">Immutable {selectedFrozen.status} review · Draft v{selectedFrozen.draft_version}</p><button className="button small" type="button" onClick={() => setSelectedFrozen(null)} disabled={lifecycleBusy}>Return to shared Draft</button>{selectedFrozen.status === "FROZEN" && canApprove ? <div className="approval-panel"><button className="button primary" type="button" onClick={() => void fileFrozen()} disabled={lifecycleBusy}>{pending === "file" ? "Filing…" : "File exact Frozen version"}</button><div className="field"><label htmlFor="change-request-comment">Required comment to request changes</label><textarea id="change-request-comment" value={changeComment} maxLength={2000} onChange={(event) => setChangeComment(event.target.value)} disabled={lifecycleBusy} /></div><button className="button" type="button" onClick={() => void requestChanges()} disabled={!changeComment.trim() || lifecycleBusy}>{pending === "changes" ? "Creating new Draft…" : "Request changes"}</button></div> : null}{["FILED", "SUPERSEDED"].includes(selectedFrozen.status) ? <div className="proof-actions">{(["md", "pdf", "xlsx"] as const).map((format) => <a className="button small" key={format} download href={`${apiBase}/api/cases/${caseId}/deliverables/by-id/${selectedFrozen.id}/export/${format}`}>{format.toUpperCase()}</a>)}</div> : <p className="muted">Downloads unlock only after independent filing.</p>}</div> : <>
          ...
          {modelSelection && registry ? <section className="scenario-insert" aria-labelledby="scenario-insert-title"><div className="section-heading"><h3 id="scenario-insert-title">Scenario Exhibit</h3><span>Temporary server calculation</span></div><div className="scenario-fields"><div className="field"><label htmlFor="scenario-assumption">Assumption</label><select id="scenario-assumption" value={scenarioForm.assumptionId} onChange={(event) => setScenarioForm((current) => ({ ...current, assumptionId: event.target.value }))} disabled={authoringLocked}>{registry.definitions.map((definition) => <option key={definition.assumption_id} value={definition.assumption_id}>{definition.label || definition.assumption_id}</option>)}</select></div><div className="field"><label htmlFor="scenario-case">Case</label><select id="scenario-case" value={scenarioForm.case} onChange={(event) => setScenarioForm((current) => ({ ...current, case: event.target.value }))} disabled={authoringLocked}><option value="BASE">Base</option><option value="DOWNSIDE">Downside</option></select></div><div className="field"><label htmlFor="scenario-period">Period</label><input id="scenario-period" value={scenarioForm.periodId} onChange={(event) => setScenarioForm((current) => ({ ...current, periodId: event.target.value }))} disabled={authoringLocked} /></div><div className="field"><label htmlFor="scenario-value">Shock value</label><input id="scenario-value" inputMode="decimal" value={scenarioForm.value} onChange={(event) => setScenarioForm((current) => ({ ...current, value: event.target.value }))} disabled={authoringLocked} /></div></div><button className="button small" type="button" onClick={() => void insertScenario()} disabled={!canWrite || authoringLocked || !scenarioForm.value}>{pending === "scenario" ? "Calculating…" : "Calculate and insert exact exhibit"}</button></section> : null}
