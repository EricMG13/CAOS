...
BRIEF = {
    ...
    "must_answer": [
        ...
        "Which downside breaks refinancing?",
    ],
...
def _runtime(
    ledgers: MemoryLedgerSet | PostgresLedgerSet, *, environment: str = "production"
) -> WorkflowRuntime:
...
def _start_research(runtime: WorkflowRuntime) -> tuple[dict[str, Any], dict[str, Any]]:
    case = runtime.runs.create_case("Deep research", "Northstar", "Services", "analyst")
    source = runtime.sources.ingest(
        {
            "case_id": case["id"],
            "filename": "source.txt",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"exclusions": ["x" * 201]},
        {
            "must_answer": [str(index) for index in range(6)],
            "exclusions": [str(index) for index in range(5)],
        },
    ],
)
...
def test_start_run_enforces_deep_research_brief_and_full_depth(
        ...
        StartRunRequest.model_validate(
            {
                ...
                "depth": "full",
                "research_brief": {**BRIEF, field: "forged"},
            }
        )


@pytest.mark.parametrize(
    "plan_hash", ["", "sha256:ABC", "sha256:" + "0" * 63, "md5:" + "0" * 64]
)
...
def test_research_plan_is_deterministic_complete_and_identity_bound() -> None:
    ...
    assigned = [
        ...
        for question in row.get("assigned_questions", [])
    ]
    ...
    assert all(
        row["source_classes"] == ["supplied_case_sources"] for row in workstreams
    )
    ...
    changed_brief = {
        **brief,
        "decision_context": "Monitor an existing first-lien position.",
    }
...
def test_research_plan_uses_main_question_when_must_answer_is_empty() -> None:
    ...
    plan, _ = bundle.plan_research(
        brief,
        "set_1",
        1,
        [{"module_id": "CP-0", "artifact_id": "art", "digest": "digest"}],
    )
...
def test_runtime_durably_pauses_after_cp0_with_server_owned_brief_and_plan() -> None:
    ...
    assert {
        key: value
        for key, value in research["budget_used"].items()
        if key != "active_minutes"
    } == {key: 0 for key in research["budget_limits"] if key != "active_minutes"}
    assert (
        0
        <= research["budget_used"]["active_minutes"]
        < research["budget_limits"]["active_minutes"]
    )
    assert research["inflight_request_digest"] is None
    assert research["attempts"] == []
    assert (
        research["proposed_plan_hash"] == f"sha256:{digest(research['proposed_plan'])}"
    )
    assert research["proposed_plan"]["source_set"] == {
        "id": source_set["id"],
    ...
    cp0_artifact = ledgers.runs.get_artifact(cp0["artifact_id"])
    assert cp0_artifact is not None
    assert research["proposed_plan"]["upstream_artifacts"] == [
        {
            "module_id": "CP-0",
            "artifact_id": cp0["artifact_id"],
            "digest": cp0_artifact["digest"],
        }
    ]
...
def test_exact_plan_approval_preserves_identity_and_phase4_fails_closed() -> None:
    ...
    try:
        ...
        identity = copy.deepcopy(
            {
                ...
                "inflight_request_digest": paused["research"][
                    "inflight_request_digest"
                ],
                "attempts": paused["research"]["attempts"],
                "upstream_artifacts": paused["research"]["proposed_plan"][
                    "upstream_artifacts"
                ],
            }
        )
        ...
        approved = runtime.approve_research_plan(
            paused["id"], "approver", paused["research"]["proposed_plan_hash"]
        )
        ...
        assert (
            ...
            == paused["research"]["proposed_plan_hash"]
        )
        ...
        assert (
            approved["research"]["proposed_plan"]["source_set"]
            == identity["source_set"]
        )
        assert approved["research"]["budget_limits"] == identity["budget_limits"]
        assert approved["research"]["budget_used"] == identity["budget_used"]
        assert (
            approved["research"]["inflight_request_digest"]
            == identity["inflight_request_digest"]
        )
        assert approved["research"]["attempts"] == identity["attempts"]
        assert (
            approved["research"]["proposed_plan"]["upstream_artifacts"]
            == identity["upstream_artifacts"]
        )
        assert (
            ledgers.publications.list_audit()[-1]["action"] == "research.plan_approved"
        )
        assert (
            ledgers.runs.get_run(paused["id"])["events"][-1]["event"]
...
def test_plan_approval_rejects_wrong_double_wrong_phase_and_changed_source_set() -> (
    None
):
    ledgers = MemoryLedgerSet()
    runtime = _runtime(ledgers)
    try:
        ...
        with pytest.raises(WorkflowError, match="PLAN_HASH_MISMATCH"):
            runtime.approve_research_plan(
                paused["id"], "approver", "sha256:" + "0" * 64
            )
...
def test_plan_approval_rejects_a_post_hash_plan_change_and_missing_pause() -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Tampered plan", "analyst")
    source_set = ledgers.sources.current_source_set(case["id"])
    ...
    changed_plan["scope"]["key"] = "changed-after-hash"
    plan_hash = f"sha256:{digest(original_plan)}"
    plan = {
        "source_set_id": source_set["id"],
        "nodes": [{"module_id": "CP-DR", "dependencies": [], "stage": 1}],
    }
    research = {
        "phase": "awaiting_approval",
        "proposed_plan": changed_plan,
        "proposed_plan_hash": plan_hash,
    }
    tampered = ledgers.runs.create_run_with_nodes(
        case["id"],
        "analyst",
        plan,
        plan["nodes"],
    ...
    missing_pause = ledgers.runs.create_run_with_nodes(
        case["id"],
        "analyst",
        plan,
        plan["nodes"],
        initial={"status": "paused", "error": None, "research": research},
    )
    runtime = _runtime(ledgers)
    try:
        with pytest.raises(WorkflowError, match="PLAN_HASH_MISMATCH"):
            runtime.approve_research_plan(tampered["id"], "approver", plan_hash)
        with pytest.raises(WorkflowError, match="PLAN_APPROVAL_NOT_AVAILABLE"):
            runtime.approve_research_plan(missing_pause["id"], "approver", plan_hash)
    finally:
        runtime.close()
...
def test_development_approval_resubmits_the_same_run() -> None:
    ...
    runtime = ResubmitRuntime(
        ...
        Settings(
            ...
            storage_dir=Path("/tmp/caos-cp-dr-resubmit"),
            deploy_v_root=DEPLOY_V,
        ),
    )
    try:
...
def test_research_planning_requires_upstream_artifact_identity() -> None:
    ...
    try:
        ...
        full = ledgers.runs.get_run(run["id"])
...
def test_postgres_research_pause_and_approval_survive_reload() -> None:
    ...
    if not database_url:
        pytest.skip(
            "CAOS_TEST_DATABASE_URL is required for durable CP-DR planning proof"
        )
    ...
    reloaded = PostgresLedgerSet(database_url)
    durable_pause = reloaded.runs.get_run(run_id)
    assert durable_pause is not None
    assert (
        durable_pause["status"] == "paused"
        and durable_pause["research"]["phase"] == "awaiting_approval"
    )
    durable_identity = copy.deepcopy(
        {
            key: durable_pause["research"][key]
            for key in (
                "brief",
                "proposed_plan",
                "budget_limits",
    ...
    resumed_ledgers = PostgresLedgerSet(database_url)
    durable_approval = resumed_ledgers.runs.get_run(run_id)
    ...
    assert {
        key: durable_approval["research"][key] for key in durable_identity
    } == durable_identity
    resumed_token = resumed_ledgers.runs.claim(run_id, "resume-proof")
    assert resumed_token is not None
    resumed_ledgers.runs.finish(run_id, resumed_token)
...
def _production_client(
    ...
    *,
    ...
    return TestClient(create_app(settings, ledgers))


def _headers(subject: str, role: str) -> dict[str, str]:
    return {
        "x-edge-authorization": "test-edge",
        "x-forwarded-user": subject,
        "x-forwarded-groups": f"caos-{role.lower()}",
    }


def _case_with_source(
    ledgers: MemoryLedgerSet, name: str, actor: str = "owner"
) -> dict[str, Any]:
    case = ledgers.runs.create_case(name, "Northstar", "Services", actor)
    ledgers.sources.ingest(
        {
            "case_id": case["id"],
            "filename": "source.txt",
            "media_type": "text/plain",
            "bytes": 8,
            "sha256": digest({"case_id": case["id"], "actor": actor}),
            "vault_path": None,
            "blocks": [],
        },
        actor,
...
def _approval_run(
    ledgers: MemoryLedgerSet, case_id: str, actor: str = "owner"
) -> tuple[str, str]:
    ...
    expected_brief = {
        **brief,
        ...
        "subject_name": ledgers.runs.get_case(case_id)["issuer"],
        "source_mode": "supplied_only",
        "research_budget": "standard",
        "plan_approval": "required",
    }
    ...
    assert research["proposed_plan"] and re.fullmatch(
        r"sha256:[0-9a-f]{64}", research["proposed_plan_hash"]
    )
    cp0 = next(node for node in paused["nodes"] if node["module_id"] == "CP-0")
    cp_dr = next(node for node in paused["nodes"] if node["module_id"] == "CP-DR")
    assert cp0["status"] == "succeeded" and cp0["artifact_id"]
...
def test_research_plan_approval_route_validates_hash_and_maps_conflicts(
    tmp_path: Path,
) -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Approval route")
    run_id, plan_hash = _approval_run(ledgers, case["id"])

    with _production_client(tmp_path, ledgers) as client:
        headers = _headers("owner", "ANALYST")
        malformed = client.post(
            f"/api/runs/{run_id}/research-plan/approve",
            headers=headers,
            json={"plan_hash": "not-a-hash"},
        )
        ...
        assert (
            wrong.status_code == 409 and wrong.json()["detail"] == "PLAN_HASH_MISMATCH"
        )
        ...
        assert (
            approved.json()["id"] == run_id
            and approved.json()["research"]["phase"] == "approved"
        )


def test_research_plan_route_denies_cross_case_outsider_and_stored_reader(
    tmp_path: Path,
) -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Target")
    other = ledgers.runs.create_case("Other", "Other", "Services", "cross-case")
    assert ledgers.runs.add_member(
        case["id"], "owner", "stored-reader", "READER", "ADMIN"
    )
    assert other["id"]
    ...
    with _production_client(tmp_path, ledgers) as client:
        ...
        outsider = client.post(
            f"/api/runs/{run_id}/research-plan/approve",
            headers=_headers("cross-case", "ADMIN"),
            json={"plan_hash": plan_hash},
        )
        ...
        for global_role in ("ANALYST", "APPROVER", "ADMIN"):
            ...
            reader = client.post(
                f"/api/runs/{run_id}/research-plan/approve",
                headers=_headers("stored-reader", global_role),
                json={"plan_hash": plan_hash},
            )
            assert reader.status_code == 403


def test_all_case_writer_role_combinations_can_approve_research_plan(
    tmp_path: Path,
) -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Writer matrix")
    with _production_client(tmp_path, ledgers) as client:
        for stored_role in ("ANALYST", "APPROVER", "ADMIN"):
            for global_role in ("ANALYST", "APPROVER", "ADMIN"):
                subject = f"{stored_role.lower()}-{global_role.lower()}"
                assert ledgers.runs.add_member(
                    case["id"], "owner", subject, stored_role, "ADMIN"
                )
                run_id, plan_hash = _approval_run(ledgers, case["id"])
                response = client.post(
                    f"/api/runs/{run_id}/research-plan/approve",
                    headers=_headers(subject, global_role),
                    json={"plan_hash": plan_hash},
                )
                assert response.status_code == 200, response.text


def test_deep_research_start_route_passes_validated_brief_to_runtime(
    tmp_path: Path,
) -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Start route")
    with _production_client(
        tmp_path, ledgers, cpdr_agent_enabled=True, cpdr_pilot_subjects=("owner",)
    ) as client:
        response = client.post(
            f"/api/cases/{case['id']}/runs",
            headers=_headers("owner", "ANALYST"),
            json={"pathway": "DEEP_RESEARCH", "depth": "full", "research_brief": BRIEF},
        )
...
def test_deep_research_availability_and_start_recheck(tmp_path: Path) -> None:
    ledgers = MemoryLedgerSet()
    case = _case_with_source(ledgers, "Availability")
    headers = _headers("owner", "ANALYST")
    ...
    with _production_client(tmp_path, ledgers) as client:
        ...
        assert (
            detail["deep_research_unavailable_reason"]
            == "Deep Research is disabled for this deployment."
        )

    with _production_client(tmp_path, ledgers, cpdr_agent_enabled=True) as client:
        detail = client.get(f"/api/cases/{case['id']}", headers=headers).json()
        assert detail["deep_research_available"] is False
        assert (
            detail["deep_research_unavailable_reason"]
            == "Deep Research is outside the pilot allowlist."
        )
        denied = client.post(
            f"/api/cases/{case['id']}/runs",
            headers=headers,
    ...
    with _production_client(
        tmp_path, ledgers, cpdr_agent_enabled=True, cpdr_pilot_case_ids=(case["id"],)
    ) as client:
        detail = client.get(f"/api/cases/{case['id']}", headers=headers).json()
        assert detail["deep_research_available"] is True
        assert detail["deep_research_unavailable_reason"] is None
