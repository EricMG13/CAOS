...
def _sheet(
    ...
    *,
    workbook_date: object = date(2026, 8, 24),
) -> None:
...
def _workbook_bytes(
    *,
    ...
    _sheet(
        workbook,
        "Hidden Support",
        [_row(borrower="Hidden", bloomberg="HIDDEN", figi="BBG00HIDDEN1")],
    )
...
def _add_source(
    store: MemoryStore, case_id: str, source_id: str, *, withdrawn: bool = False
) -> None:
...
def test_cp3_workbook_maps_all_visible_sector_rows_with_source_units() -> None:
    content = _workbook_bytes(
        first_rows=[
            _row(margin=400, change_1d=0.5),
            _row(
                borrower="Apex Group",
                bloomberg="BLS5005287",
                figi="BBG01S807689",
                margin="#N/A",
                change_1d="N/A",
            ),
    ...
    assert [row["sector"] for row in parsed["rows"]] == [
        "Healthcare IT",
        "IT Services",
        "IT Services",
    ]
    access = next(
        row for row in parsed["rows"] if row["borrower_name"] == "Access CIG LLC"
    )
    ...
    assert (
        parsed["universe_digest"]
        == parse_loan_workbook(content, source_id="src_1", source_sha256="a" * 64)[
            "universe_digest"
        ]
    )
...
def test_conflicting_duplicates_and_identifier_mappings_reject_the_candidate() -> None:
    ...
    with pytest.raises(LoanWorkbookValidationError) as raised:
        parse_loan_workbook(
            _workbook_bytes(first_rows=[_row(), conflict, mapping_conflict]),
            source_id="src",
            source_sha256="c" * 64,
        )
...
def test_partial_headers_and_conflicting_dates_reject_the_complete_workbook() -> None:
    ...
    assert {"RV_TEMPLATE_PARTIAL", "RV_TEMPLATE_MISSING"}.issubset(
        _codes(header_error.value)
    )
...
def test_invalid_rows_fail_closed(row: list[object], expected_code: str) -> None:
    with pytest.raises(LoanWorkbookValidationError) as raised:
        parse_loan_workbook(
            _workbook_bytes(first_rows=[row]), source_id="src", source_sha256="f" * 64
        )
...
def test_formula_without_cached_value_becomes_null_without_execution() -> None:
    ...
    parsed = parse_loan_workbook(
        _workbook_bytes(first_rows=[formula_row]),
        source_id="src",
        source_sha256="1" * 64,
    )
...
def test_memory_store_versions_idempotently_supersedes_and_withdraws_loan_universes() -> (
    None
):
    store = MemoryStore()
    ...
    first_parsed = parse_loan_workbook(
        _workbook_bytes(), source_id="src_1", source_sha256="1" * 64
    )
    first, created = store.save_loan_universe_import(
        _import_record(store, case_id, first_parsed), first_parsed["rows"], "analyst"
    )
    duplicate, duplicate_created = store.save_loan_universe_import(
        _import_record(store, case_id, first_parsed), first_parsed["rows"], "analyst"
    )
    ...
    assert (
        first["version"] == 1
        and store.active_loan_universe(case_id)["id"] == first["id"]
    )

    second_parsed = parse_loan_workbook(
        _workbook_bytes(first_rows=[_row(margin=425)]),
        source_id="src_2",
        source_sha256="2" * 64,
    )
    second, second_created = store.save_loan_universe_import(
        _import_record(store, case_id, second_parsed), second_parsed["rows"], "analyst"
    )
    ...
    rejected, rejected_created = store.save_loan_universe_import(
        _import_record(
            ...
            rejected_parsed,
            status="REJECTED",
            findings=[{"code": "RV_TEMPLATE_PARTIAL"}],
        ),
        [],
        "analyst",
    )
    assert (
        rejected_created is True
        and rejected["status"] == "REJECTED"
        and rejected["version"] is None
    )
    assert store.active_loan_universe(case_id)["id"] == second["id"]

    with store.lock:
        withdrawn_id = store._withdraw_loan_universe_for_source_locked(
            case_id, "src_2", "analyst"
        )
        store.persist()
...
def test_memory_store_rolls_back_complete_loan_state_when_persistence_fails() -> None:
    ...
    parsed = parse_loan_workbook(
        _workbook_bytes(), source_id="src", source_sha256="4" * 64
    )
    store.fail = True

    with pytest.raises(RuntimeError, match="database unavailable"):
        store.save_loan_universe_import(
            _import_record(store, case_id, parsed), parsed["rows"], "analyst"
        )
...
def test_store_rejects_a_source_withdrawn_while_its_workbook_was_parsing() -> None:
    ...
    parsed = parse_loan_workbook(
        _workbook_bytes(), source_id="src", source_sha256="5" * 64
    )
...
def test_loan_universe_source_foreign_key_migration_backfills_before_validation() -> (
    None
):
    migration = (
        Path(__file__).parents[1]
        / "server"
        / "migrations"
        / "004_rv_loan_universe_source_fk.sql"
    ).read_text()
...
def test_postgres_loan_import_converges_and_normalizes_rows() -> None:
    ...
    if not database_url:
        pytest.skip(
            "CAOS_TEST_DATABASE_URL is required for durable loan-universe proof"
        )
    first_store = PostgresStore(database_url)
    case = first_store.create_case(
        "Loan RV", first_store._id("issuer"), "Services", "analyst"
    )
    ...
    parsed = parse_loan_workbook(
        _workbook_bytes(),
        source_id=source_id,
        source_sha256=first_store.sources[source_id]["sha256"],
    )
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(
            executor.map(
                lambda item: item[0].save_loan_universe_import(
    ...
    assert (
        active is not None
        and active["id"] == universe_id
        and len(active["rows"]) == parsed["row_count"]
    )
...
def test_postgres_import_and_source_withdrawal_converge_without_active_residue() -> (
    None
):
    database_url = os.getenv("CAOS_TEST_DATABASE_URL")
    if not database_url:
    ...
    case = import_store.create_case(
        "Loan withdrawal race", import_store._id("issuer"), "Services", "analyst"
    )
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        withdrawal = executor.submit(
            withdrawal_store.withdraw_source, case["id"], source_id, "analyst"
        )
...
def _client(tmp_path: Path, ledgers: MemoryLedgerSet | None = None) -> TestClient:
    settings = Settings(
    ...
    return TestClient(create_app(settings, ledgers or MemoryLedgerSet()))
...
def _upload_workbook(
    ...
    case_id: str,
    content: bytes,
    name: str = "REF_CP-3_Sector_RV.xlsx",
) -> dict[str, object]:
...
def test_case_api_imports_reads_and_idempotently_returns_the_active_universe(
    tmp_path: Path,
) -> None:
    with _client(tmp_path) as client:
        case_id = client.post(
            "/api/cases",
            json={"name": "Loan RV", "issuer": "Issuer", "sector": "Services"},
        ).json()["id"]
        source = _upload_workbook(client, case_id, _workbook_bytes())

        imported = client.post(
            f"/api/cases/{case_id}/rv/loan-universes", json={"source_id": source["id"]}
        )
...
def test_invalid_import_returns_structured_findings_and_preserves_prior_active_universe(
    tmp_path: Path,
) -> None:
    with _client(tmp_path) as client:
        case_id = client.post(
        ...
        active_id = client.post(
            f"/api/cases/{case_id}/rv/loan-universes",
        ...
        workbook.close()
        invalid_source = _upload_workbook(
            client, case_id, invalid_bytes.getvalue(), "changed-template.xlsx"
        )

        rejected = client.post(
            f"/api/cases/{case_id}/rv/loan-universes",
            json={"source_id": invalid_source["id"]},
        )
        repeated = client.post(
            f"/api/cases/{case_id}/rv/loan-universes",
            json={"source_id": invalid_source["id"]},
        ...
        assert {
            finding["code"] for finding in rejected.json()["detail"]["findings"]
        } >= {"RV_TEMPLATE_PARTIAL", "RV_TEMPLATE_MISSING"}
        assert (
            repeated.json()["detail"]["universe_id"]
            == rejected.json()["detail"]["universe_id"]
        )
    assert active["universe"]["id"] == active_id


def test_import_rejects_vault_bytes_that_do_not_match_the_source_digest(
    tmp_path: Path,
) -> None:
    ledgers = MemoryLedgerSet()
    with _client(tmp_path, ledgers) as client:
        case_id = client.post(
            "/api/cases",
            json={"name": "Loan RV", "issuer": "Issuer", "sector": "Services"},
        ).json()["id"]
        source = _upload_workbook(client, case_id, _workbook_bytes())
        vault_path = (
            tmp_path / "vault" / "sources" / source["sha256"][:2] / source["sha256"]
        )
        ...
        assert ledgers.sources.active_loan_universe(case_id) is None
...
def test_loan_universe_api_is_case_scoped_and_reader_safe(tmp_path: Path) -> None:
    ...
    with _client(tmp_path, ledgers) as client:
        ...
        other_case_id = client.post(
            "/api/cases",
            json={"name": "Other", "issuer": "Other", "sector": "Services"},
        ).json()["id"]
        source = _upload_workbook(client, case_id, _workbook_bytes())
        assert (
            client.post(
                f"/api/cases/{other_case_id}/rv/loan-universes",
        ...
        assert ledgers.runs.add_member(
            case_id, "local-analyst", "reader", "READER", "ADMIN"
        )
        reader_headers = {"x-forwarded-user": "reader", "x-caos-role": "READER"}
        assert (
            client.get(
                f"/api/cases/{case_id}/rv/loan-universes/active", headers=reader_headers
            ).status_code
            == 200
        )
        assert (
        ...
        assert (
            client.get(
                f"/api/cases/{case_id}/rv/loan-universes/active",
...
def test_cp3_artifact_binds_the_pinned_normalized_loan_universe(tmp_path: Path) -> None:
    with _client(tmp_path) as client:
        ...
        started = client.post(
            ...
            json={"pathway": "RELATIVE_VALUE", "depth": "full"},
        )
        ...
        cp3 = client.app.state.ledgers.runs.get_artifact(cp3_node["artifact_id"])
        ...
        assert cp3["payload"]["inputs"]["loan_universe"] == {
            "identity": identity,
            "rows": active_rows,
        }
        assert {row["source_locators"][0]["sheet"] for row in active_rows} == {
            "IT Services",
            "Healthcare IT",
        }
