...
def create_app(
    ...
    ledgers: MemoryLedgerSet | PostgresLedgerSet | None = None,
) -> FastAPI:
    ...
    sources = ledgers.sources
    runs = ledgers.runs
    publications = ledgers.publications
    ...
    model_readiness = ModelReadinessService(runs, sources, models, bundle, model_bundle)
    ...
    def get_run_or_404(run_id: str, actor: Identity) -> dict[str, Any]:
        ...
        require_case(runs, run["case_id"], actor)
        return run

    def model_for_case(case_id: str, build_id: str) -> dict[str, Any]:
        build = models.get_build(build_id)
    ...
    def health() -> dict[str, Any]:
        return {
            "status": "ok",
            "service": "caos",
            "bundle_build_id": bundle.build_id,
    ...
    def cases(request: Request) -> list[dict[str, Any]]:
        return runs.list_cases(identity(request).subject)
    ...
    def create_case(payload: CreateCaseRequest, request: Request) -> dict[str, Any]:
        ...
        return runs.create_case(
            payload.name, payload.issuer, payload.sector, who.subject
        )

    @app.get("/api/cases/{case_id}", response_model=CaseDetailResponse)
    def case_detail(case_id: str, request: Request) -> dict[str, Any]:
        who = identity(request)
        case = require_case(runs, case_id, who)
        latest = runs.latest_run(case_id)
        deep_research_available = settings.cpdr_agent_enabled and (
            case_id in settings.cpdr_pilot_case_ids
            or who.subject in settings.cpdr_pilot_subjects
        )
        ...
        return {
            **case,
            "source_set": current_source_set(sources, case_id),
            "source_count": len(list_sources(sources, case_id)),
            "pathway_fit": pathway_fit(sources, case_id),
            "accepted_snapshot": accepted_snapshot(runs, case_id),
            "latest_run": latest,
            "deep_research_available": deep_research_available,
            "deep_research_unavailable_reason": deep_research_unavailable_reason,
        }
    ...
    def add_member(
        case_id: str, payload: MemberRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        if not runs.get_case(case_id):
            raise HTTPException(status_code=404, detail="case not found")
        if who.role != "ADMIN":
            require_case(runs, case_id, who, write=True)
        if not runs.add_member(
            case_id, who.subject, payload.subject, payload.role.value, who.role
        ):
            raise HTTPException(
                status_code=403, detail="case membership authority required"
            )
        return {
            "case_id": case_id,
    ...
    def case_sources(case_id: str, request: Request) -> list[dict[str, Any]]:
        require_case(runs, case_id, identity(request))
        return list_sources(sources, case_id)

    @app.post(
        "/api/cases/{case_id}/sources", status_code=201, response_model=SourceResponse
    )
    async def upload_source(
        case_id: str, request: Request, file: UploadFile
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
    ...
    @app.get(
        "/api/cases/{case_id}/artifacts/{artifact_id}", response_model=ArtifactResponse
    )
    def artifact_detail(
        case_id: str, artifact_id: str, request: Request
    ) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        artifact = runs.get_artifact(artifact_id)
        if not artifact or artifact.get("case_id") != case_id:
            raise HTTPException(status_code=404, detail="artifact not found")
        return artifact

    @app.post(
        "/api/cases/{case_id}/sources/{source_id}/withdraw",
        response_model=SourceResponse,
    )
    def withdraw_source(
        case_id: str, source_id: str, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        source = sources.withdraw(case_id, source_id, who.subject)
        if source is None:
            raise HTTPException(status_code=404, detail="source not found")
        return source
    ...
    def fit(case_id: str, request: Request) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        return pathway_fit(sources, case_id)

    @app.get("/api/cases/{case_id}/runs", response_model=list[RunRouteResponse])
    def case_runs(case_id: str, request: Request) -> list[dict[str, Any]]:
        require_case(runs, case_id, identity(request))
        return runs.list_runs(case_id)

    @app.post(
        "/api/cases/{case_id}/runs", status_code=202, response_model=RunRouteResponse
    )
    def start_run(
        case_id: str, payload: StartRunRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        if payload.pathway == "DEEP_RESEARCH":
            if not settings.cpdr_agent_enabled:
                raise HTTPException(
                    status_code=403,
                    detail="Deep Research is disabled for this deployment.",
                )
            if (
                case_id not in settings.cpdr_pilot_case_ids
                and who.subject not in settings.cpdr_pilot_subjects
            ):
                raise HTTPException(
                    status_code=403,
                    detail="Deep Research is outside the pilot allowlist.",
                )
        try:
            research_brief = (
                payload.research_brief.model_dump(mode="json")
    ...
    @app.post(
        "/api/runs/{run_id}/upgrade", status_code=202, response_model=RunRouteResponse
    )
    def upgrade_run(run_id: str, request: Request) -> dict[str, Any]:
        who = identity(request)
        previous = get_run_or_404(run_id, who)
        require_case(runs, previous["case_id"], who, write=True)
        if previous["plan"]["depth"] != Depth.SCREEN.value:
            raise HTTPException(
                status_code=409, detail="only Screen runs can be upgraded"
            )
        try:
            return runtime.start_run(
                previous["case_id"],
                who.subject,
                previous["plan"]["pathway"],
                Depth.FULL,
                previous["plan"].get("focus_questions", []),
                upgraded_from_run_id=previous["id"],
            )
        except MethodologyError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
    ...
    def run_events(run_id: str, request: Request) -> StreamingResponse:
        ...
        return StreamingResponse(
            runtime.stream_events(run_id, cursor),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
        )

    @app.post(
        "/api/runs/{run_id}/research-plan/approve", response_model=RunRouteResponse
    )
    def approve_research_plan(
        run_id: str, payload: ApproveResearchPlanRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
    ...
    def snapshot(case_id: str, request: Request) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        current = accepted_snapshot(runs, case_id)
        latest_snapshot = latest_accepted_snapshot(runs, case_id)
        return {
            "accepted": current,
            "latest_accepted": latest_snapshot,
            "diff": snapshot_diff(current, latest_snapshot)
            if latest_snapshot
            else None,
            "switch_required": bool(
                current and latest_snapshot and current["id"] != latest_snapshot["id"]
            ),
        }

    @app.post("/api/cases/{case_id}/snapshot/switch", response_model=SnapshotResponse)
    def switch_snapshot(
        case_id: str, payload: SnapshotSwitchRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        target = runs.switch_visible_snapshot(case_id, payload.snapshot_id, who.subject)
        if target is None:
    ...
    def lens(case_id: str, request: Request) -> dict[str, Any]:
        case = require_case(runs, case_id, identity(request))
        accepted = accepted_snapshot(runs, case_id)
        return {
            "issuer": case["issuer"],
            "sector": case["sector"],
            "accepted_snapshot_id": accepted["id"] if accepted else None,
            "accepted_snapshot_digest": accepted["digest"] if accepted else None,
            "source_set": current_source_set(sources, case_id),
        }
    ...
    @app.post(
        "/api/cases/{case_id}/report-inputs",
        status_code=201,
        response_model=ReportInputsResponse,
    )
    def report_inputs(
        case_id: str, payload: ReportInputsRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
    ...
    @app.post(
        "/api/cases/{case_id}/thesis", status_code=201, response_model=ThesisResponse
    )
    def write_thesis(
        case_id: str, payload: ThesisRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
            return save_thesis(publications, case_id, who.subject, payload)
        except ValueError as exc:
            if str(exc) == "VERSION_CONFLICT":
                raise HTTPException(
                    status_code=409, detail="thesis version changed"
                ) from exc
            raise HTTPException(status_code=422, detail=str(exc)) from exc
    ...
    def recommendations(case_id: str, request: Request) -> dict[str, Any]:
        ...
        return {
            ...
            "current": recommendation_state(
                runs, case_id, versions[-1] if versions else None
            ),
        }
    ...
    def write_recommendations(
        case_id: str, payload: RecommendationMatrixRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
            ...
            return save_recommendations(
                ...
                who.subject,
                payload,
                accepted["id"] if accepted else None,
            )
        except ValueError as exc:
            if str(exc) == "VERSION_CONFLICT":
                raise HTTPException(
                    status_code=409, detail="recommendation version changed"
                ) from exc
            raise HTTPException(status_code=422, detail=str(exc)) from exc
    ...
    @app.post(
        "/api/cases/{case_id}/notes", status_code=201, response_model=NoteRouteResponse
    )
    def write_note(
        case_id: str, payload: NoteRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        return create_note(publications, case_id, who.subject, payload.body)

    @app.post(
        "/api/cases/{case_id}/notes/{note_id}/promote", response_model=NoteRouteResponse
    )
    def promote(case_id: str, note_id: str, request: Request) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
            return promote_note(publications, case_id, note_id, who.subject)
    ...
    @app.get(
        "/api/cases/{case_id}/assumptions", response_model=list[AssumptionResponse]
    )
    ...
    @app.post(
        "/api/cases/{case_id}/assumptions",
        status_code=201,
        response_model=AssumptionResponse,
    )
    def write_assumption(
        case_id: str, payload: AssumptionRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
    ...
    def rv(case_id: str, request: Request) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        return compare_universe(publications, case_id, accepted_snapshot(runs, case_id))

    @app.post(
        "/api/cases/{case_id}/rv", status_code=201, response_model=RVUniverseResponse
    )
    def write_rv(
        case_id: str, payload: RVUniverseRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        try:
    ...
    @app.get(
        "/api/cases/{case_id}/rv/loan-universes/active",
        response_model=ActiveLoanUniverseRouteResponse,
    )
    def active_loan_universe(case_id: str, request: Request) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        active = sources.active_loan_universe(case_id)
        if not active:
    ...
    def import_case_loan_universe(
        ...
        try:
            record, created = import_loan_source(
                sources, case_id, payload.source_id, who.subject
            )
    ...
    def case_models(case_id: str, request: Request) -> dict[str, Any]:
        require_case(runs, case_id, identity(request))
        return {
            "readiness": model_readiness.readiness(case_id),
            "builds": [
                public_model_build(build) for build in models.list_builds(case_id)
            ],
        }

    @app.post(
        "/api/cases/{case_id}/models",
        status_code=202,
        response_model=QueueModelResponse,
    )
    ...
    @app.get(
        ...
        response_model=ModelWorksheetResponse,
    )
    def model_worksheet(
    ...
    @app.post(
        "/api/cases/{case_id}/models/{build_id}/export",
        status_code=202,
        response_model=QueueModelExportResponse,
    )
    def queue_model_export(
        ...
        try:
            build, queued = models.queue_export(build_id, who.subject)
        except ValueError as exc:
            raise HTTPException(
                status_code=409, detail="MODEL_EXPORT_NOT_READY"
            ) from exc
    ...
    @app.post(
        "/api/cases/{case_id}/reports/freeze",
        status_code=201,
        response_model=ReportRouteResponse,
    )
    def freeze(
        case_id: str, payload: FreezeReportRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
        snapshot_value = accepted_snapshot(runs, case_id)
        thesis_value = next(
            (
                value
                for value in publications.list_theses(case_id)
                if value["version"] == payload.thesis_version
            ),
            None,
        )
        recommendation_value = recommendation_state(
            runs,
            case_id,
            next(
                (
                    value
                    for value in publications.list_recommendations(case_id)
                    if value["version"] == payload.recommendation_version
                ),
                None,
            ),
        )
    ...
    @app.post(
        "/api/cases/{case_id}/reports/approve", response_model=ReportRouteResponse
    )
    def approve(
        case_id: str, payload: ApproveRequest, request: Request
    ) -> dict[str, Any]:
        who = identity(request)
        require_case(runs, case_id, who, write=True)
    ...
    def export_report(case_id: str, format_name: str, request: Request) -> Response:
        ...
        if not report or report["status"] != "APPROVED":
            raise HTTPException(
                status_code=409, detail="approved frozen report required"
            )
        if format_name == "md":
            return Response(
                report["markdown"],
    ...
    def admin_bundle(request: Request) -> dict[str, Any]:
        ...
        return {
            ...
            "integrity": bundle.verify(),
            "drafts": publications.list_methodology_drafts(),
        }
    ...
    @app.post(
        "/api/admin/drafts", status_code=201, response_model=MethodologyDraftResponse
    )
    def create_methodology_draft(
        payload: MethodologyDraftRequest, request: Request
    ) -> dict[str, Any]:
        who = admin_step_up(request)
        module_ids = {item["module_id"] for item in bundle.catalog["modules"]}
        if (
            payload.expected_build_id != bundle.build_id
            or payload.module_id not in module_ids
        ):
            raise HTTPException(
                status_code=422, detail="draft authority identity is not current"
            )
        if payload.before == payload.after:
            raise HTTPException(
                status_code=422, detail="draft must change a governed value"
            )
        return publications.create_methodology_draft(
            {
    ...
    @app.post(
        "/api/admin/drafts/{draft_id}/validate", response_model=MethodologyDraftResponse
    )
    def validate_methodology_draft(draft_id: str, request: Request) -> dict[str, Any]:
        ...
        draft = next(
            (
                value
                for value in publications.list_methodology_drafts()
                if value["id"] == draft_id
            ),
            None,
        )
        if not draft:
        ...
        if (
            draft["expected_build_id"] != bundle.build_id
            or draft["before"] == draft["after"]
        ):
            raise HTTPException(
                status_code=422,
    ...
    @app.post(
        "/api/admin/drafts/{draft_id}/confirm", response_model=MethodologyDraftResponse
    )
    def confirm_methodology_draft(
        draft_id: str, payload: ConfirmDraftRequest, request: Request
    ) -> dict[str, Any]:
        who = admin_step_up(request)
        draft = next(
            (
        ...
        return publications.confirm_methodology_draft(draft_id, who.subject, signature)

    @app.post(
        "/api/admin/recipes/validate", response_model=VisualRecipeValidationResponse
    )
    def validate_visual_recipe(
