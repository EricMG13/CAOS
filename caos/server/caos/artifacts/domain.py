...
def save_thesis(
    publications: PublicationLedger, case_id: str, actor: str, request: ThesisRequest
) -> dict[str, Any]:
...
def save_recommendations(
    ...
    return publications.append_recommendations(
        ...
        request.expected_version,
        {
            ...
            "analytical_dependency_ids": list(request.analytical_dependency_ids),
...
def save_report_inputs(
    ...
    return publications.save_report_inputs(
        ...
        actor,
        {
            "expected_version": thesis_request.expected_version,
        ...
        {
            "expected_version": recommendation_request.expected_version,
            "market_snapshot_id": recommendation_request.market_snapshot_id,
            "rows": [
                row.model_dump(mode="json") for row in recommendation_request.rows
            ],
            "analytical_dependency_ids": list(
                recommendation_request.analytical_dependency_ids
            ),
        },
...
def build_snapshot_payload(
    ...
    if run.get("status") != "succeeded" or len(run.get("nodes", [])) != len(
        run.get("node_ids", [])
    ):
        raise ValueError("RUN_NOT_READY")
    canonical_required = is_canonical_full_credit(run.get("plan") or {})
    canonical_runner = (
        CanonicalModuleRunner(bundle)
        if canonical_required and bundle is not None
        else None
    )
    ...
    for node in run["nodes"]:
        artifact = (
            runs.get_artifact(node.get("artifact_id"))
        ...
        if (
            ...
            raise ValueError("RUN_NOT_READY")
        if node.get("module_id") == "CP-DR" and not cpdr_artifact_is_valid(
            runs, sources, run, node, artifact, bundle
        ):
            raise ValueError("RUN_NOT_READY")
    ...
    if canonical_required:
        try:
            if canonical_runner is None or set(canonical_artifacts) != set(
                CANONICAL_MODULES
            ):
                raise ValueError("RUN_NOT_READY")
...
def canonical_artifact_is_valid(
    ...
    try:
        ...
        if not isinstance(evidence_refs, list) or any(
            not isinstance(row, dict) for row in evidence_refs
        ):
            return False
        evidence_keys = [
            (row.get("source_id"), row.get("block_id")) for row in evidence_refs
        ]
        ...
        for row in evidence_refs:
            try:
                evidence = sources.read_pinned_evidence(
                    run["case_id"],
                    source_set["id"],
            ...
            expected_evidence[-1]["locator"] = json.dumps(
                expected_evidence[-1]["locator"],
                sort_keys=True,
                separators=(",", ":"),
            )
            expected_evidence[-1]["extractor_version"] = str(
                expected_evidence[-1]["extractor_version"]
            )
            expected_evidence[-1]["confidence"] = str(
                expected_evidence[-1]["confidence"]
            )
        ...
        if (
            ...
            or confidence["qa_status"] != "Passed"
        ):
            return False
...
def cpdr_artifact_is_valid(
    ...
    try:
        ...
        if (
            ...
            or approved_plan.get("source_set")
            != {"id": source_set["id"], "version": source_set["version"]}
        ...
        for row in transport.get("evidence", []):
            ...
            blocks = [
                ...
                if item.get("block_id") == row.get("block_id")
            ]
            if len(blocks) != 1:
                return False
            block = blocks[0]
        ...
        if payload.model_dump(mode="json") != transport:
            return False
        confidence = bundle.cpdr_confidence(
            confidence_inputs(payload, returned_evidence)
        )
        if envelope["host_confidence"] != confidence:
            return False
        filename, markdown = render_cpdr_markdown(
            payload, confidence, run["created_at"][:10], expected_upstream
        )
        canonical_output = envelope["canonical_output"]
        validation = bundle.validate_cpdr_handoff(
            markdown, filename, run["id"], brief["as_of_date"]
        )
        return bool(
...
def snapshot_diff(
    ...
    return {
        "changed": old != new
        or previous.get("source_set_id") != current.get("source_set_id"),
        "added": [
            {"module_id": k, "digest": v} for k, v in new.items() if k not in old
        ],
        "removed": [
            {"module_id": k, "digest": v} for k, v in old.items() if k not in new
        ],
        "modified": [
            {"module_id": k, "before": old[k], "after": v}
            for k, v in new.items()
            if k in old and old[k] != v
        ],
        "source_set_changed": previous.get("source_set_id")
        != current.get("source_set_id"),
    }
...
def accepted_snapshot(runs: RunLedger, case_id: str) -> dict[str, Any] | None:
    ...
    snapshot_id = (
        (case.get("visible_snapshot_id") or case.get("accepted_snapshot_id"))
        if case
        else None
    )
...
def latest_accepted_snapshot(runs: RunLedger, case_id: str) -> dict[str, Any] | None:
    case = runs.get_case(case_id)
    return (
        runs.get_snapshot(case["accepted_snapshot_id"])
        if case and case.get("accepted_snapshot_id")
        else None
    )
