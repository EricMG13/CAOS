...
class _MemoryState:
    """Private state shared by the four in-memory ledger adapters."""

    def __init__(self, lease_seconds: float) -> None:
        # ponytail: one lock is deliberate; split per ledger only after measured contention.
        self.lock = threading.RLock()
        self.lease_seconds = lease_seconds
        self.cases: dict[str, Record] = {}
        self.sources: dict[str, Record] = {}
        self.source_sets: dict[str, Record] = {}
        self.source_set_history: dict[str, Record] = {}
        self.runs: dict[str, Record] = {}
        self.nodes: dict[str, Record] = {}
        self.artifacts: dict[str, Record] = {}
        self.snapshots: dict[str, Record] = {}
        self.theses: dict[str, list[Record]] = {}
        self.recommendations: dict[str, list[Record]] = {}
        self.notes: dict[str, list[Record]] = {}
        self.assumptions: dict[str, list[Record]] = {}
        self.reports: dict[str, Record] = {}
        self.methodology_drafts: dict[str, Record] = {}
        self.rv_universes: dict[str, Record] = {}
        self.rv_loan_universes: dict[str, Record] = {}
        self.rv_loan_rows: dict[str, list[Record]] = {}
        self.rv_active_loan_universes: dict[str, str] = {}
        self.audit: list[Record] = []
        self.events: dict[str, list[Record]] = {}
    ...
    def _queue_model_build_locked(
        ...
        case = self.cases.get(build.get("case_id"))
        ...
        if (
            ...
            or not case
            or case.get("accepted_snapshot_id") != build.get("accepted_snapshot_id")
            or (not run)
    ...
    def _complete_model_job_locked(
        self, build_id: str, key: str, result: dict[str, Any], actor: str, kind: str
    ) -> dict[str, Any]:
    ...
    def _fail_model_job_locked(
        self, build_id: str, key: str, error: dict[str, Any], actor: str, kind: str
    ) -> dict[str, Any]:
        if (
    ...
    def save_loan_universe_import(
        self, record: dict[str, Any], rows: list[dict[str, Any]], actor: str
    ) -> tuple[dict[str, Any], bool]:
        with self.lock:
            prior = copy.deepcopy(
                (
