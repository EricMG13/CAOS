...
## 2026-08-24 — CP-MODEL end-to-end deployment design critic pass

Decision under review: replace the signed-authority placeholder with canonical Full Credit handoffs, an asynchronous CP-MODEL export, immutable persistence, and case-scoped download.

| ID | Perspective | Objection | Impact | Status | Resolution / disposition |
|----|-------------|-----------|--------|--------|--------------------------|
| RT-2026-08-24-122 | Methodology-authority reviewer | The current Full Credit route emits CP-2A, but CP-MODEL requires a distinct CP-2B handoff. Treating CP-2A as CP-2B would recreate the authority defect under a new label. | Critical | Resolved in design | CAOS leaves the vendored bundle unchanged and derives a separate CP-2B projection only from CP-2A's binding absorbed phase. The projection records its source artifact, performs no analysis, and must pass CP-2B plus CP-MODEL validation. Missing projection content remains `NOT_READY`. |
| RT-2026-08-24-123 | Product-integrity reviewer | Wiring the exporter to today's generic host artifacts would make the UI appear deployed while every real build either fails or consumes invented values. | Critical | Resolved in design | Full Credit activates canonical CP-1, CP-1A, CP-1B, CP-2, and CP-2A handoffs first. Generic placeholders cannot satisfy snapshot acceptance or model readiness. The final signed-authority text is removed only after a real end-to-end workbook passes. |
| RT-2026-08-24-124 | Concurrency reviewer | Reusing workflow jobs for model builds would violate their run foreign key and allow duplicate workers or stale attempts to publish conflicting metadata. | High | Resolved in design | Migration `002` adds model-specific records and leased jobs with one fingerprint per case. PostgreSQL transactions fence claims, renewals, takeover, and finalization; identical requests converge on one build. |
| RT-2026-08-24-125 | Filesystem attacker | A build or issuer identifier could become a path, formula, or filename injection, and a retry could overwrite a prior workbook. | Critical | Resolved in design | API input never selects a path or filename. CP-MODEL validates the issuer token, writes through a private temporary directory, and publishes to a server-derived build directory with exclusive creation. Downloads resolve only recorded relative vault keys and verify hash and size. |
| RT-2026-08-24-126 | Transaction reviewer | Filesystem publication and database readiness cannot share one transaction. A crash can leave an orphan file or ready metadata pointing to no file. | High | Resolved in design | The worker publishes under an immutable build ID before fenced metadata finalization. An orphan is not API-addressable; the same build retry verifies and adopts it. Downloads recheck file presence, size, and hash before streaming. |
| RT-2026-08-24-127 | Data-governance reviewer | Activating canonical modules could send all case evidence to an external provider or let provider-authored lineage become authoritative. | Critical | Resolved in design | The worker reuses the bounded evidence-read tool and worker-only provider key. It exposes only pinned evidence blocks and named methodology sections. The host owns source set, identity, evidence lineage, provenance, hashes, validators, and artifact acceptance. |
| RT-2026-08-24-128 | Resource-exhaustion reviewer | Large modules and LibreOffice can exhaust worker memory, disk, or time, blocking unrelated analytical runs. | High | Resolved in design | Provider work retains bounded budgets and leases. Every export uses a private temporary directory, bounded executor capacity, exclusive output, and terminal cleanup. Production verification includes concurrent analytical and model work under the existing worker cap. |
| RT-2026-08-24-129 | Authorization reviewer | Guessable build IDs or a valid workbook path could permit cross-case metadata access or download. | Critical | Resolved in design | Every status, queue, and download route applies case membership independently. Queueing requires write authority. The server verifies build-to-case ownership and ready-state file integrity before streaming. |
| RT-2026-08-24-130 | Report-governance reviewer | A report could freeze against one snapshot and later include a model from another, or approval could ignore the model change. | Critical | Resolved in design | Report freeze accepts only a ready build with the same accepted snapshot and stores its build ID, hash, and input fingerprint. Approval recomputes the complete report fingerprint with that immutable model identity. |
